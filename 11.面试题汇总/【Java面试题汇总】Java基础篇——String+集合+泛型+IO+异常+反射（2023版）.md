...
**String为什么不可被继承：** 因为String类是由final修饰的，所以String类不可被继承。

**String字符串为什么不可被变：**  因为String底层char类型的value数组是private final修饰的，final修饰导致value不能指向新数组（但无法保证value这个引用变量指向的真实数组不可变），private修饰且没对外暴露任何修改value的方法导致value这个引用变量指向的真实数组不可变。
...
> 结果：
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/3f95fa94e1f049fe8be16384d62f3764.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
...
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/75e3c04a18194691b5358058f91943d0.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
...
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/3cf789ce71d5475f9d3496a84d0887f3.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
...
![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/97ea93a253522b8008fafd72007239bc.jpeg)
...
>
> 具体地说，epoll调用使用了一组系统调用**(epoll_create、epoll_ctl和epoll_wait)**来实现I/O多路复用机制。当应用程序调用epoll_wait时，内核会等待任何一个被监控的文件描述符上的事件（例如套接字事件、管道事件等）。此时**内核会把就绪的事件记入到内存中的一个事件列表中**，应用程序再通过epoll_wait函数**返回这个就绪事件列表**。在整个等待过程中，应用程序并不需要自己不停地轮询文件描述符集合，这样可以极大地减少系统调用和CPU资源的占用。
>
> 总之，epoll调用不需要轮询整个文件描述符集合，这是它相比于select和poll等调用的一个关键优势。因此，在高并发的网络应用程序中，epoll调用更加适用且更加高效。
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/d32d81f2b5b249bb9e924b9a7abc3ddd.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
>
> 
>
...
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/118f745ea7be4249a2592f881c196590.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
>
> 
>
> **NIO，非阻塞I/O模型（nonblocking I/O）：**recvfrom从应用层到内核的时候,如果该缓冲区没有数据的话,就直接返回一个EWOULDBLOCK错误,一般都对非阻塞I/O模型进行轮询检查这个状态,看内核是不是有数据到来。
>
> ![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/53e935d32a4a46e8ac51eff919edfc56.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
>
> 
>
...
![img](【Java面试题汇总】Java基础篇——String+集合+泛型+IO+异常+反射（2023版）.assets/e03c5bac3af249919acdb8671cfcaf98.png)![点击并拖拽以移动](data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==)
...
[【2023Java面试八股文】Java多线程篇_vincewm的博客-CSDN博客](https://blog.csdn.net/qq_40991313/article/details/129446871)